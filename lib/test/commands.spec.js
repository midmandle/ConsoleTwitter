...
describe('CommandProcessor', function () {
	...
	it('readTimeline(user) should print the timeline of the user.', function() {
		const user = new User('Alice');
		const message = 'A message from Alice';
		user.timeline.addNewPost(message);

		const mockedGetFormattedTimelines = sinon.fake();
		user.timeline.getFormattedPosts = mockedGetFormattedTimelines;

		const mockedPrintToConsole = sinon.fake();
		commandProcessor.printToConsole = mockedPrintToConsole;

		commandProcessor.readTimeline(user);

		expect(mockedGetFormattedTimelines.called);
		expect(mockedPrintToConsole.called);
	});
	...
	it('viewWall(user) should print all the posts of the user and anyone they follow.', function() {
		const userA = new User('Alice');
		...
		const mockedGetWallPosts = sinon.fake();
		userA.getWallPosts = mockedGetWallPosts;

		const mockedPrintToConsole = sinon.fake();
		commandProcessor.printToConsole = mockedPrintToConsole;

		commandProcessor.viewWall(userA);

		expect(mockedGetWallPosts.called);
		expect(mockedPrintToConsole.called);
	});
